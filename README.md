...
> 详细更新日志见 [CHANGELOG](./CHANGELOG.md)

### v1.4.0 — 卡片布局重设计 & 海外独立模式

- 同好会卡片列表重构：头像左上对齐、全宽分割线、联系信息/外部链接在左下角
- 默认排序改为 vnfest → region → school
- 海外同好会成为独立国家模式，不再归属于中国省份
- 修复日本地图渲染崩溃（`d is undefined`）
- 归类整理 `.gitignore`
