(() => {
  ...
  async function loadEvents() {
    try {
      // ✅ 改成 galgame_events.json
      const resp = await fetch('./galgame_events.json', { cache: 'no-store' });
      if (!resp.ok) return;
  ...
  function renderSelectedDayEvents(dateKey, eventMap) {
    ...
    if (!dayEvents.length) {
      ...
      // ✅ 修改文案
      els.calendarEventList.innerHTML = `<div class="calendar-empty">本月有${monthEventCount}个Galgame活动</div>`;
      return;
    }
  ...
  function renderCalendar(options = {}) {
    ...
    // ✅ 修改标题
    els.calendarTitle.textContent = `${year}年${month + 1}月 Galgame活动日历`;
    ...
    if (!monthHasAnyEvent) {
      // ✅ 修改文案
      els.calendarEventList.innerHTML = '<div class="calendar-empty">本月暂无Galgame活动</div>';
      return;
    }
